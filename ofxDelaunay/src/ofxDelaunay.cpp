...
int ofxDelaunay::triangulate(){
 ...
 //-------------------Original-------------------
int ofxDelaunay::triangulate(vector<ofFloatColor>& colors){
    ...
    for (int i = 0; i < nv; i++){
        triangleMesh.addVertex(ofVec3f(vertices[i].x,vertices[i].y,vertices[i].z));
        triangleMesh.addColor(colors.at(i));
    }
...
void ofxDelaunay::draw(){
	if(ofGetStyle().bFill){
	    triangleMesh.drawFaces();
    }
    else{
        triangleMesh.clearColors();
        triangleMesh.setColorForIndices(0, ntri, ofColor(50,100));
        ofSetLineWidth(.001f);
